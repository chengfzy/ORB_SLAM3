...
int main(int argc, char** argv) {
    ...
    if (argc < 6) {
        cerr << endl
             << "Usage: ./mono_inertial_tum_vi path_to_vocabulary path_to_settings path_to_image_folder_1 "
                "path_to_times_file_1 path_to_imu_data_1 (path_to_image_folder_2 path_to_times_file_2 "
                "path_to_imu_data_2 ... path_to_image_folder_N path_to_times_file_N path_to_imu_data_N) "
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        LoadImages(string(argv[3 * (seq + 1)]), string(argv[3 * (seq + 1) + 1]), vstrImageFilenames[seq],
                   vTimestampsCam[seq]);
        cout << "LOADED!" << endl;
        ...
        while (vTimestampsImu[seq][first_imu[seq]] <= vTimestampsCam[seq][0]) first_imu[seq]++;

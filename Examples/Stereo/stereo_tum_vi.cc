...
int main(int argc, char** argv) {
    ...
    string file_name;
    if (bFileName) file_name = string(argv[argc - 1]);

    if (argc < 6) {
        cerr << endl
             << "Usage: ./stereo_tum_vi path_to_vocabulary path_to_settings path_to_image_folder1_1 "
                "path_to_image_folder2_1 path_to_times_file_1 (path_to_image_folder1_2 path_to_image_folder2_2 "
                "path_to_times_file_2 ... path_to_image_folder1_N path_to_image_folder2_N path_to_times_file_N) "
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        LoadImages(string(argv[(3 * seq) + 3]), string(argv[(2 * seq) + 4]), string(argv[(2 * seq) + 5]),
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
            ...
            if (imLeft.empty() || imRight.empty()) {
                cerr << endl << "Failed to load image at: " << vstrImageLeftFilenames[seq][ni] << endl;
                return 1;
            }

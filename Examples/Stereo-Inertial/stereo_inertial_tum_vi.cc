...
int main(int argc, char** argv) {
    ...
    if (argc < 7) {
        cerr << endl
             << "Usage: ./stereo_inertial_tum_vi path_to_vocabulary path_to_settings path_to_image_folder_1 "
                "path_to_image_folder_2 path_to_times_file path_to_imu_data (trajectory_file_name)"
             << endl;
        return 1;
    }
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        LoadImages(string(argv[4 * (seq + 1) - 1]), string(argv[4 * (seq + 1)]), string(argv[4 * (seq + 1) + 1]),

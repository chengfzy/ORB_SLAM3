...
KeyFrame::KeyFrame()
    : mnFrameId(0),
      ...
      /*mBowVec(NULL), mFeatVec(NULL),*/ mnScaleLevels(0),
      mfScaleFactor(0),
      mfLogScaleFactor(0),
      mvScaleFactors(0),
      mvLevelSigma2(0),
...
void KeyFrame::SetPose(const cv::Mat& Tcw_) {
    ...
    cv::Mat Rwc = Rcw.t();
    Ow = -Rwc * tcw;
    if (!mImuCalib.Tcb.empty()) Owb = Rwc * mImuCalib.Tcb.rowRange(0, 3).col(3) + Ow;
...
void KeyFrame::EraseMapPointMatch(MapPoint* pMP) {
    ...
    if (rightIndex != -1) mvpMapPoints[rightIndex] = static_cast<MapPoint*>(NULL);
}

void KeyFrame::ReplaceMapPointMatch(const int& idx, MapPoint* pMP) { mvpMapPoints[idx] = pMP; }
...
void KeyFrame::UpdateConnections(bool upParent) {
    ...
    for (vector<MapPoint*>::iterator vit = vpMP.begin(), vend = vpMP.end(); vit != vend; vit++) {
        ...
        for (map<KeyFrame*, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end();
             mit != mend; mit++) {
            if (mit->first->mnId == mnId || mit->first->isBad() || mit->first->GetMap() != mpMap) continue;
            KFcounter[mit->first]++;
        }
    }
    ...
    for (map<KeyFrame*, int>::iterator mit = KFcounter.begin(), mend = KFcounter.end(); mit != mend; mit++) {
        if (!upParent) cout << "  UPDATE_CONN: KF " << mit->first->mnId << " ; num matches: " << mit->second << endl;
        if (mit->second > nmax) {
            nmax = mit->second;
            pKFmax = mit->first;
        }
...
cv::Matx44f KeyFrame::GetRightPose_() {
    ...
    cv::Matx44f Trw{Rrw(0, 0), Rrw(0, 1), Rrw(0, 2), trw(0), Rrw(1, 0), Rrw(1, 1), Rrw(1, 2), trw(1),
                    Rrw(2, 0), Rrw(2, 1), Rrw(2, 2), trw(2), 0.f,       0.f,       0.f,       1.f};

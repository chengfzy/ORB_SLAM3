...
void LoopClosing::Run() {
    ...
    while (1) {
        ...
        if (CheckNewKeyFrames()) {
            ...
            double timeDetect = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                                    time_EndDetectBoW - time_StartDetectBoW)
                                    .count();
            ...
            if (bDetected) {
                if (mbMergeDetected) {
                    if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO) &&
                        ...
                        Verbose::PrintMess(
                            "Number of KFs in the current map: " + to_string(mpCurrentKF->GetMap()->KeyFramesInMap()),
                            Verbose::VERBOSITY_DEBUG);
                        cv::Mat mTmw = mpMergeMatchedKF->GetPose();
                        g2o::Sim3 gSmw2(Converter::toMatrix3d(mTmw.rowRange(0, 3).colRange(0, 3)),
                                        Converter::toVector3d(mTmw.rowRange(0, 3).col(3)), 1.0);
                        cv::Mat mTcw = mpCurrentKF->GetPose();
                        g2o::Sim3 gScw1(Converter::toMatrix3d(mTcw.rowRange(0, 3).colRange(0, 3)),
                        ...
                        double timeMerge = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                ...
                if (mbLoopDetected) {
                    ...
                    if (mpCurrentKF->GetMap()->IsInertial()) {
                        ...
                        g2o::Sim3 g2oTwc(Converter::toMatrix3d(Twc.rowRange(0, 3).colRange(0, 3)),
                        ...
                        if (fabs(phi(0)) < 0.008f && fabs(phi(1)) < 0.008f && fabs(phi(2)) < 0.349f) {
                            ...
                            double timeLoop = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
...
#ifdef REGISTER_TIMES
        std::chrono::steady_clock::time_point time_EndDetectBoW = std::chrono::steady_clock::now();
        timeDetectBoW = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(time_EndDetectBoW -
                                                                                               time_StartDetectBoW)
                            .count();
#endif
    }
        ...
        if (!bAbortByNearKF && numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
        {
            ...
            if (bConverge) {
                ...
                g2o::Sim3 gScm(Converter::toMatrix3d(solver.GetEstimatedRotation()),
                               Converter::toVector3d(solver.GetEstimatedTranslation()), solver.GetEstimatedScale());
                    ...
                    if (numOptMatches >= nSim3Inliers) {
                        ...
                        if (numProjOptMatches >= nProjOptMatches) {
                            ...
                            while (nNumKFs < 3 && j < vpCurrentCovKFs.size()) {
                                ...
                                g2o::Sim3 gSjc(Converter::toMatrix3d(mTjc.rowRange(0, 3).colRange(0, 3)),
...
void LoopClosing::CorrectLoop() {
    ...
    // Optimize graph
    bool bFixedScale = mbFixScale;
    if (mpTracker->mSensor == System::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2()) bFixedScale = false;
...
void LoopClosing::MergeLocal() {
    ...
    if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO) {
        Optimizer::MergeInertialBA(mpLocalMapper->GetCurrKF(), mpMergeMatchedKF, &bStop, mpCurrentKF->GetMap(),
                                   vCorrectedSim3);
    } else {
    ...
    if (vpCurrentMapKFs.size() == 0) {
        ...
        Verbose::PrintMess("MERGE: Calculate the new position of the elements outside of the window",
                           Verbose::VERBOSITY_DEBUG);
        // Apply the transformation
        {
            if (mpTracker->mSensor == System::MONOCULAR) {
                ...
                for (KeyFrame* pKFi : vpCurrentMapKFs) {
                    ...
                    if (pCurrentMap->isImuInitialized()) {
                        Eigen::Matrix3d Rcor = eigR.transpose() * vNonCorrectedSim3[pKFi].rotation().toRotationMatrix();
                        pKFi->SetVelocity(Converter::toCvMat(Rcor) *
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF) {
    ...
    double timeFullGBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(time_StartMapUpdate - time_StartFGBA)
            .count();
        ...
        if (!mbStopGBA) {
            ...
            while (!lpKFtoCheck.empty()) {
                ...
                if (pKF->bImu) {
                    ...
                    if (pKF->mVwbGBA.empty()) Verbose::PrintMess("pKF->mVwbGBA is empty", Verbose::VERBOSITY_NORMAL);
    ...
    double timeMapUpdate =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(time_EndMapUpdate - time_StartMapUpdate)
            .count();
    vTimeMapUpdate_ms.push_back(timeMapUpdate);

    double timeGBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(time_EndMapUpdate - time_StartFGBA)
            .count();

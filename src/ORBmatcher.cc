                ...
                for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++) {
                    ...
                    if (dist < bestDist) {
                        ...
                        bestLevel = (F.Nleft == -1)
                                        ...
                                        : (idx < F.Nleft) ? F.mvKeys[idx].octave : F.mvKeysRight[idx - F.Nleft].octave;
                        bestIdx = idx;
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, const cv::Mat& F12,
                                       const KeyFrame* pKF2, const bool b1) {
...
bool ORBmatcher::CheckDistEpipolarLine2(const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, const cv::Mat& F12,
...
int ORBmatcher::SearchForTriangulation(KeyFrame* pKF1, KeyFrame* pKF2, cv::Mat F12,
    ...
    if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
        ...
        tlr = pKF1->GetRotation() * (-pKF2->GetRightRotation().t() * pKF2->GetRightTranslation()) +
              pKF1->GetTranslation();
        trl = pKF1->GetRightRotation() * (-pKF2->GetRotation().t() * pKF2->GetTranslation()) +
              pKF1->GetRightTranslation();
        trr = pKF1->GetRightRotation() * (-pKF2->GetRightRotation().t() * pKF2->GetRightTranslation()) +
              pKF1->GetRightTranslation();
    }
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                const cv::KeyPoint& kp1 =
                    (pKF1->NLeft == -1)
                        ? pKF1->mvKeysUn[idx1]
                        : (idx1 < pKF1->NLeft) ? pKF1->mvKeys[idx1] : pKF1->mvKeysRight[idx1 - pKF1->NLeft];
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                    ...
                    if (pCamera1->epipolarConstrain(pCamera2, kp1, kp2, R12, t12, pKF1->mvLevelSigma2[kp1.octave],
...
int ORBmatcher::SearchForTriangulation_(KeyFrame* pKF1, KeyFrame* pKF2, cv::Matx33f F12,
    ...
    if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
        ...
        tlr = pKF1->GetRotation_() * (-pKF2->GetRightRotation_().t() * pKF2->GetRightTranslation_()) +
              pKF1->GetTranslation_();
        trl = pKF1->GetRightRotation_() * (-pKF2->GetRotation_().t() * pKF2->GetTranslation_()) +
              pKF1->GetRightTranslation_();
        trr = pKF1->GetRightRotation_() * (-pKF2->GetRightRotation_().t() * pKF2->GetRightTranslation_()) +
              pKF1->GetRightTranslation_();
    }
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                    ...
                    if (pCamera1->epipolarConstrain_(pCamera2, kp1, kp2, R12, t12, pKF1->mvLevelSigma2[kp1.octave],
                                                     pKF2->mvLevelSigma2[kp2.octave]) ||
                        bCoarse)  // MODIFICATION_2
                    {
                        bestIdx2 = idx2;
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                    ...
                    cv::Mat x3D;
                    if (pCamera1->matchAndtriangulate(kp1, kp2, pCamera2, Tcw1, Tcw2, pKF1->mvLevelSigma2[kp1.octave],
                                                      pKF2->mvLevelSigma2[kp2.octave], x3D)) {
                        bestIdx2 = idx2;
                        bestDist = dist;
                        bestPoint = x3D;
                    }
    ...
    for (int i = 0; i < nMPs; i++) {
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++) {
            ...
            const cv::KeyPoint& kp =
                (pKF->NLeft == -1) ? pKF->mvKeysUn[idx] : (!bRight) ? pKF->mvKeys[idx] : pKF->mvKeysRight[idx];
            ...
            if (!LastFrame.mvbOutlier[i]) {
                ...
                if (CurrentFrame.Nleft != -1) {
                    ...
                    if (bForward)
                        ...
                        vIndices2 =
                            CurrentFrame.GetFeaturesInArea(uv.x, uv.y, radius, nLastOctave - 1, nLastOctave + 1, true);
                    ...
                    for (vector<size_t>::const_iterator vit = vIndices2.begin(), vend = vIndices2.end(); vit != vend;
                        ...
                        if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft])
                            if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft]->Observations() > 0) continue;
        ...
        if (pMP) {
            if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                ...
                const vector<size_t> vIndices2 =
                    CurrentFrame.GetFeaturesInArea(uv.x, uv.y, radius, nPredictedLevel - 1, nPredictedLevel + 1);

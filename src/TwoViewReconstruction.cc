...
float TwoViewReconstruction::CheckHomography(const cv::Mat& H21, const cv::Mat& H12, vector<bool>& vbMatchesInliers,
                                             float sigma) {
    const int N = mvMatches12.size();
...
void TwoViewReconstruction::Triangulate(const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, const cv::Mat& P1,
...
int TwoViewReconstruction::CheckRT(const cv::Mat& R, const cv::Mat& t, const vector<cv::KeyPoint>& vKeys1,

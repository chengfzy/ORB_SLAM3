...
VertexVelocity::VertexVelocity(KeyFrame* pKF) { setEstimate(Converter::toVector3d(pKF->GetVelocity())); }

VertexVelocity::VertexVelocity(Frame* pF) { setEstimate(Converter::toVector3d(pF->mVw)); }

VertexGyroBias::VertexGyroBias(KeyFrame* pKF) { setEstimate(Converter::toVector3d(pKF->GetGyroBias())); }
...
VertexAccBias::VertexAccBias(KeyFrame* pKF) { setEstimate(Converter::toVector3d(pKF->GetAccBias())); }
...
void EdgeInertial::linearizeOplus() {
    ...
    _jacobianOplus[0].block<3, 3>(6, 0) =
        Skew(Rbw1 * (VP2->estimate().twb - VP1->estimate().twb - VV1->estimate() * dt - 0.5 * g * dt * dt));  // OK
    // translation
...
void EdgeInertialGS::linearizeOplus() {
    ...
    _jacobianOplus[0].block<3, 3>(6, 0) =
        Skew(Rbw1 * (s * (VP2->estimate().twb - VP1->estimate().twb - VV1->estimate() * dt) - 0.5 * g * dt * dt));
    // translation
...
// SO3 FUNCTIONS
Eigen::Matrix3d ExpSO3(const Eigen::Vector3d& w) { return ExpSO3(w[0], w[1], w[2]); }
...
Eigen::Matrix3d InverseRightJacobianSO3(const Eigen::Vector3d& v) { return InverseRightJacobianSO3(v[0], v[1], v[2]); }
...
Eigen::Matrix3d RightJacobianSO3(const Eigen::Vector3d& v) { return RightJacobianSO3(v[0], v[1], v[2]); }

...
bool sortByVal(const pair<MapPoint*, int>& a, const pair<MapPoint*, int>& b) { return (a.second < b.second); }
...
void Optimizer::BundleAdjustment(const vector<KeyFrame*>& vpKFs, const vector<MapPoint*>& vpMP, int nIterations,
    ...
    for (size_t i = 0; i < vpMP.size(); i++) {
        ...
        for (map<KeyFrame*, tuple<int, int>>::const_iterator mit = observations.begin(); mit != observations.end();
            ...
            if (optimizer.vertex(id) == NULL || optimizer.vertex(pKF->mnId) == NULL) continue;
            nEdges++;
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, vector<KeyFrame*>& vpNonEnoughOptKFs) {
    ...
    for (int i = 0, iend = vNeighKFs.size(); i < iend; i++) {
        ...
        if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) lLocalKeyFrames.push_back(pKFi);
    }
    ...
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++) {
        ...
        for (map<KeyFrame*, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end();
            ...
            if (pKFi->mnBALocalForKF != pKF->mnId && pKFi->mnBAFixedForKF != pKF->mnId) {
                ...
                if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) lFixedCameras.push_back(pKFi);
            }
        }
    }
    ...
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    if (pCurrentMap->IsInertial()) solver->setUserLambdaInit(100.0);  // TODO uncomment
...
void Optimizer::OptimizeEssentialGraph6DoF(KeyFrame* pCurKF, vector<KeyFrame*>& vpFixedKFs,
...
void Optimizer::OptimizeEssentialGraph(KeyFrame* pCurKF, const LoopClosing::KeyFrameAndPose& NonCorrectedSim3,
                                       const LoopClosing::KeyFrameAndPose& CorrectedSim3) {
    // Setup optimizer
    Map* pMap = pCurKF->GetMap();
    g2o::SparseOptimizer optimizer;
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2, vector<MapPoint*>& vpMatches1,
                            vector<KeyFrame*>& vpMatches1KF, g2o::Sim3& g2oS12, const float th2, const bool bFixScale,
...
Eigen::MatrixXd Optimizer::Sparsify(const Eigen::MatrixXd& H, const int& start1, const int& end1, const int& start2,
                                    const int& end2) {
...
void Optimizer::InertialOptimization(Map* pMap, Eigen::Matrix3d& Rwg, double& scale, Eigen::Vector3d& bg,
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            if (pKFi->isBad() || pKFi->mPrevKF->mnId > maxKFid) continue;
            if (!pKFi->mpImuPreintegrated) std::cout << "Not preintegrated measurement" << std::endl;
...
void Optimizer::InertialOptimization(vector<KeyFrame*> vpKFs, Eigen::Vector3d& bg, Eigen::Vector3d& ba, float priorG,
                                     float priorA) {
    int its = 200;
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF, vector<KeyFrame*> vpWeldingKFs,
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF, vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF,
    ...
    if (bDoMore) {
        ...
        Verbose::PrintMess("LBA: First optimization, there are " + to_string(badMonoMP) + " monocular and " +
    ...
    Verbose::PrintMess("LBA: Second optimization, there are " + to_string(badMonoMP) + " monocular and " +
                           to_string(badStereoMP) + " sterero bad edges",
                       Verbose::VERBOSITY_DEBUG);
    ...
    if (!vToErase.empty()) {
        ...
        for (MapPoint* pMPi : spErasedMPs) {
            if (pMPi->isBad()) {
                Verbose::PrintMess("LBA: MP " + to_string(pMPi->mnId) +
        ...
        for (KeyFrame* pKFi : spErasedKFs) {
            ...
            Verbose::PrintMess("LBA: Initially KF " + to_string(pKFi->mnId) + " had " + to_string(num_init_MPs) +
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool* pbStopFlag, Map* pMap,
    ...
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++) {
        ...
        for (map<KeyFrame*, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
            ...
            if ((pKFi->mnBALocalForKF != pCurrKF->mnId) && (pKFi->mnBAFixedForKF != pCurrKF->mnId)) continue;
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame* pFrame, bool bRecInit) {
    ...
    pFrame->SetImuPoseVelocity(Converter::toCvMat(VP->estimate().Rwb), Converter::toCvMat(VP->estimate().twb),

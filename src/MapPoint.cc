...
MapPoint::MapPoint(const cv::Mat& Pos, Map* pMap, Frame* pFrame, const int& idxF)
    ...
    const int level = (pFrame->Nleft == -1)
                          ...
                          : (idxF < pFrame->Nleft) ? pFrame->mvKeys[idxF].octave : pFrame->mvKeysRight[idxF].octave;
...
void MapPoint::UpdateNormalAndDepth() {
...

    {
        ...
        mNormalVectorx =
            cv::Matx31f(mNormalVector.at<float>(0), mNormalVector.at<float>(1), mNormalVector.at<float>(2));
    }
}

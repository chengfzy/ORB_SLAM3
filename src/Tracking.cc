...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Atlas* pAtlas,
                   KeyFrameDatabase* pKFDB, const string& strSettingPath, const int sensor, const string& _nameSeq)
...
void Tracking::TrackStats2File() {
    ...
    f << "#KF insert[ms], ORB ext[ms], Stereo match[ms], IMU preint[ms], Pose pred[ms], LM track[ms], KF dec[ms], "
         "Total[ms]"
      << endl;
    ...
    for (int i = 0; i < vnKeyFramesLM.size(); ++i) {
        f << vnKeyFramesLM[i] << "," << vnMapPointsLM[i] << "," << vdUpdatedLM_ms[i] << "," << vdSearchLP_ms[i] << ","
          << vdPoseOpt_ms[i] << endl;
    }
...
bool Tracking::ParseCamParamFile(cv::FileStorage& fSettings) {
    ...
    if (sCameraName == "PinHole") {
        ...
        if (mDistCoef.rows == 5) std::cout << "- k3: " << mDistCoef.at<float>(4) << std::endl;
...
void Tracking::PreintegrateIMU() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!mpImuPreintegratedFromLastKF) cout << "mpImuPreintegratedFromLastKF does not exist" << endl;
...
void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        // Save frame if recent relocalization, since they are used for IMU reset (as we are making copy, it shluld be
        // once mCurrFrame is completely modified)
        if ((mCurrentFrame.mnId < (mnLastRelocFrameId + mnFramesToResetIMU)) &&
        ...
        // Update drawer
        mpFrameDrawer->Update(this);
        if (!mCurrentFrame.mTcw.empty()) mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);
...
void Tracking::MonocularInitialization() {
    if (!mpInitializer) {
        ...
        if (mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn, mCurrentFrame.mvKeysUn, mvIniMatches, Rcw, tcw,
...
void Tracking::CreateInitialMapMonocular() {
    ...
    if (mSensor == System::IMU_MONOCULAR) pKFini->mpImuPreintegrated = (IMU::Preintegrated*)(NULL);
...
void Tracking::CreateMapInAtlas() {
    ...
    if (mSensor == System::IMU_STEREO || mSensor == System::IMU_MONOCULAR) mpAtlas->SetInertialSensor();
    mbSetInit = false;
...
bool Tracking::TrackLocalMap() {
    ...
    double timeUpdatedLM_ms =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_StartSearchLP - time_StartLMUpdate)
            .count();
    ...
    double timeSearchLP_ms =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_StartPoseOpt - time_StartSearchLP)
            .count();
    ...
    double timePoseOpt_ms =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndPoseOpt - time_StartPoseOpt)
            .count();
...
bool Tracking::NeedNewKeyFrame() {
    ...
    if (mpLastKeyFrame) {
        if (mSensor == System::IMU_MONOCULAR) {
            if ((mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.5) c3 = true;
        } else if (mSensor == System::IMU_STEREO) {
            if ((mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.5) c3 = true;
        }
    }
    ...
    if ((((mnMatchesInliers < 75) && (mnMatchesInliers > 15)) || mState == RECENTLY_LOST) &&
        ((mSensor ==
          System::IMU_MONOCULAR)))  // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) ||

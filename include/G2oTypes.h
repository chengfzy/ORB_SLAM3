...
class EdgeStereoOnlyPose : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexPose> {
  public:
    ...
    EdgeStereoOnlyPose(const cv::Mat& Xw_, int cam_idx_ = 0) : Xw(Converter::toVector3d(Xw_)), cam_idx(cam_idx_) {}
...
class ConstraintPoseImu {
  public:
    ...
    ConstraintPoseImu(const cv::Mat& Rwb_, const cv::Mat& twb_, const cv::Mat& vwb_, const IMU::Bias& b,
                      const cv::Mat& H_) {
